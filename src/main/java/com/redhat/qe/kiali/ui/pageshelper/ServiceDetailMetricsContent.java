...
public class ServiceDetailMetricsContent extends RootPage {
    ...
    private static final int EXPECTED_CARD_SIZE = 4;
    ...
    public ServiceDetailMetricsContent(KialiWebDriver driver, Tab rootTab) {
        super(driver);
    ...
    private void loadThisPage(boolean force) {
        if (force || !rootTab.selected().equals(TAB.METRICS.getText())) {
            rootTab.select(TAB.METRICS.getText());
